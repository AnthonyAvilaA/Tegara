...
class CanvasDrawCommand(Command, MouseListener):
    def __init__(self, canvas: Canvas, color: Color, position: Point, draw_size: int, line_density_factor = 0.4, max_points_for_line: int = 60, optimization_factor: float = 3, smoothing_factor: float = 0.15) -> None:
        ...
        self._prev_position: list[Point] = [position]
        self._draw_size = draw_size
        self._changed_pixels: set[CanvasPixel] = set()
        self._points_since_last_update = 0
        self._optimization_factor = optimization_factor
        self._max_points_for_line = max_points_for_line
        self._line_density_factor = line_density_factor
        self._smooth_position = position # El punto suavizado que realmente dibuja
        self._smoothing_factor = smoothing_factor    # Entre 0 y 1. Menos es más suave pero con más "delay"
        self._changed_coords_set = set()
    
    def execute(self):
        if self._changed_pixels:
            self.redo()
            return
            
        image = self._canvas.get_image()
        self.draw_point(self._position, image)
        self._canvas.set_image(image)
    
    def redo(self):
        image = self._canvas.get_image()
        color_list = self._color.get_list()
        for pixel in self._changed_pixels:
    ...
    def on_mouse_event(self, event: Event) -> None:
        ...
        new_smooth_x = self._smooth_position.get_x() + \
                    (target_position.get_x() - self._smooth_position.get_x()) * self._smoothing_factor
        new_smooth_y = self._smooth_position.get_y() + \
                    (target_position.get_y() - self._smooth_position.get_y()) * self._smoothing_factor
        ...
        for point in __line_points:
            ...
            self._points_since_last_update += 1
        
        if self._points_since_last_update > self._draw_size * self._optimization_factor:
            self._canvas.set_image(image)
            self._points_since_last_update = 0

        self._prev_position.append(self._position)
        if len(self._prev_position) > 5:
            self._prev_position.pop(0)
        self._position = new_smooth_pos
        
        self._smooth_position = Point(new_smooth_x, new_smooth_y) # Guardar con decimales para precisión
    ...
    def draw_point(self, point: Point, image: ndarray) -> None:
        canvas_w = self._canvas.get_width()
        canvas_h = self._canvas.get_height()
        r = self._draw_size // 2
        ...
        for iy, ix in zip(indices_y, indices_x):
                ay, ax = y0 + iy, x0 + ix
                if (ax, ay) not in self._changed_coords_set:
                    # Solo guardamos lo estrictamente necesario
                    old_c = image[ay, ax].copy()
                    self._changed_pixels.add(CanvasPixel(Point(ax, ay), Color(*old_c)))
                    self._changed_coords_set.add((ax, ay))

        # 5. Aplicar el color de una sola vez usando la máscara
        roi[mask] = self._color.get_list()

    def __line_points(self, new_position: Point) -> list[Point]:
        points = []

        p0 = self._prev_position[-2] if len(self._prev_position) >= 2 else self._prev_position[-1]
        p1 = self._prev_position[-1]
        p2 = self._position
        p3 = new_position
        ...
        dynamic_density = self._line_density_factor
        if dist > self._draw_size:
            dynamic_density *= 1.5

        # Calcular STEPS dinámicos
        STEPS = int(dist * dynamic_density)
        STEPS = max(10, min(self._max_points_for_line, STEPS))  # clamp
